...
const Dashboard = () => {
  ...
  const fetchExpenseandIncome = async (type) => {
    ...
    try {
      let expensesdata = await axios.get(`${URL}/api/expenses/${type}`, config);
      setExpenses(expensesdata.data);
      let incomedata = await axios.get(`${URL}/api/income/${type}`, config);
  ...
  const dateSearchHandeler = async () => {
    ...
    try {
      let expensesdata = await axios.get(
        `${URL}/api/expenses?startDate=${startDate}&endDate=${endDate}`,
        config
      );
      setExpenses(expensesdata.data);
      let incomedata = await axios.get(
        `${URL}/api/income?startDate=${startDate}&endDate=${endDate}`,
        config
      );
      setIncome(incomedata.data);
      ...
      {ctx.user && (
        <>
          <div className="dashboard-nav">
            <div style={{ flex: 2 }}>
              <h3 style={{ textAlign: "center" }}>EI-TRACKER</h3>
            </div>
